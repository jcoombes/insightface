...
class Operator:
    ...
    def create_syn(self, face, trg_angle=[0, 0, 0], include_mask=None):
        ...
        if face.exclude_mask is not None:
            tcoord_sampling = np.round(self.tcoords.points[:,::-1] * face.exclude_mask.shape).astype(np.int64)
    ...
    def run(self, im, reconstruction_dict, face_mask=None):
        ...
        tcoord_sampling = np.round(self.tcoords.points*angle_uv_src.shape).astype(np.int64)
