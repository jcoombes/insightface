...
def get_image(roidb, scale=False):
    ...
    for i in range(num_images):
        ...
        if 'boxes_mask' in roi_rec:
            ...
            boxes_mask = boxes_mask.astype(np.int64)
...
def __get_crop_image(roidb):
    ...
    for i in range(num_images):
        ...
        im = cv2.resize(im,
                        ...
                        interpolation=cv2.INTER_LINEAR)
        new_rec['boxes'] = roi_rec['boxes'].copy() * im_scale
        box_scale = new_rec['boxes'][box_ind].copy().astype(np.int64)
        ...
        #tbox = np.array([left, left+SIZE, up, up+SIZE], dtype=np.int64)
        ...
        new_rec['boxes'][:, 1] -= up
        new_rec['boxes'][:, 3] -= up
        box_trans = new_rec['boxes'][box_ind].copy().astype(np.int64)
        ...
        if DEBUG:
            ...
            if TMP_ID < 10:
                tim = im.copy()
                for i in range(new_rec['boxes'].shape[0]):
                    box = new_rec['boxes'][i].copy().astype(np.int64)
